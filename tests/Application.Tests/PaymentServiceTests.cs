...
public class PaymentServiceTests
{
    ...
    public PaymentServiceTests()
    {
        ...
        _sut = new PaymentService(_unitOfWorkMock.Object, _paymentGatewayMock.Object);
    }
    ...
    [Fact]
    public async Task QueryDebtAsync_WhenNotPaid_ShouldReturnExactSubscriptionPrice()
    {
        var now = DateTime.UtcNow;
        var subscription = new Subscription
        {
        ...
        result.Should().NotBeNull();
        result!.Amount.Should().Be(15.99m);
        result.DueDate.Should().Be(subscription.NextPaymentDate);
        result.Period.Should().Be($"{now.Year} {now.Month:D2}");
    }

    [Fact]
    public async Task QueryDebtAsync_WhenSubscriptionNotFound_ShouldThrowKeyNotFoundException()
    {
        _subscriptionRepoMock
            .Setup(r => r.GetByIdAsync(999))
            .ReturnsAsync((Subscription?)null);

        var act = async () => await _sut.QueryDebtAsync(999);
